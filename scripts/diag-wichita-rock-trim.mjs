#!/usr/bin/env node
/**
 * Wichita rock-trim diagnostics on Variant B (MARKET_BP_PATCH + dialBpAmToFm).
 * In-vm only — does not modify shipped legacy.js.
 *
 *   node scripts/diag-wichita-rock-trim.mjs
 *
 * B = Variant B baseline | E/F/G/H = rock trims on top of B
 */
...
import { classifyTop40Mismatch, expectedFormatLeadershipProfile } from './expectedFormatLeadershipProfile.mjs';
...
const WICHITA_BP_PATCH_B = `  wichita:{
...
/** E: weaker ALBUM_ROCK seed (BP FM AOR slots). */
const WICHITA_BP_PATCH_E = `  wichita:{
...
const OPENING_SHAPE_F = `${OPENING_SHAPE_ANCHOR}
    if(marketId==='wichita'){
      if(['ALBUM_ROCK','CLASSIC_ROCK'].includes(s.format)&&s.sig?.type==='FM')f*=0.86;
    }`;

const APPL_DRIFT_DAMP_ANCHOR = `  if(dr&&dr.cfg){
    if(s.format==='TOP40')
      driftMod=hitsTop40DemoEffect(dr.val,coh,year);
    else if(dr.cfg.demoEffect)
      driftMod=Math.max(0.5,dr.cfg.demoEffect(dr.val,coh));
  }`;

const APPL_DRIFT_DAMP_G = `${APPL_DRIFT_DAMP_ANCHOR}
  if(marketId==='wichita'&&year>=1995&&['ALBUM_ROCK','CLASSIC_ROCK','ALT_ROCK','AAA'].includes(s.format)){
    driftMod*=0.90;
  }
  let wichitaRockBookTrimMult=1;
  if(marketId==='wichita'&&year>=1995&&['ALBUM_ROCK','CLASSIC_ROCK','ALT_ROCK','AAA'].includes(s.format)){
    wichitaRockBookTrimMult=0.90;
  }`;

const APPL_OUT_ANCHOR =
...
const APPL_OUT_G = `const out=Math.max(0, aff * q * eff * amP * atl * sp * sat * strm * simBonus * driftMod * morHeritageHybridMult * hitsLineageEraMult * chrLineageBucketAppealEraMult01 * eraMult * oldiesAgeMult * fmMusPref * fmLeaderAppealTrim * franchiseDemoMult(s,coh,G) * mktFmt * allNewsSig * zombieNicheMult * staffingAutomationAppealTradeoffMult(s,G)*brokeredAppealTradeoffMult(s,G)*aaaEraAppealMult*(typeof wichitaRockBookTrimMult==='number'?wichitaRockBookTrimMult:1));`;
...
function applyVariantBBase(src) {
  let out = src;
...
function patchLegacyForVariant(src, variant) {
  let out = injectHeadlessMegaFragNewsGuard(applyVariantBBase(src));

  if (variant === 'E' || variant === 'H') {
    out = out.replace(WICHITA_BP_PATCH_B, WICHITA_BP_PATCH_E);
  }

  if (variant === 'F' || variant === 'H') {
    if (!out.includes("['ALBUM_ROCK','CLASSIC_ROCK'].includes(s.format)")) {
      out = out.replace(OPENING_SHAPE_ANCHOR, OPENING_SHAPE_F);
    }
  }

  if (variant === 'G' || variant === 'H') {
    if (!out.includes('wichitaRockBookTrimMult')) {
      out = out.replace(APPL_DRIFT_DAMP_ANCHOR, APPL_DRIFT_DAMP_G);
      out = out.replace(APPL_OUT_ANCHOR, APPL_OUT_G);
    }
  }
...
function variantLabel(v) {
  if (v === 'B') return 'B Variant B baseline';
  if (v === 'E') return 'E weaker ALBUM_ROCK BP str (niche)';
  if (v === 'F') return 'F rock FM opening OQ ×0.86';
  if (v === 'G') return 'G post-1995 rock drift/book damp';
  return 'H combined E+F+G';
}

function score2026(m) {
  if (!m) return -999;
  let s = 0;
  if (m.countryPct >= 12 && m.countryPct <= 20) s += 3;
  else if (m.countryPct >= 10) s += 1;
  if (m.rockPct >= 12 && m.rockPct <= 18) s += 3;
  else if (m.rockPct <= 22) s += 1;
  if (m.acPct >= 15 && m.acPct <= 24) s += 1;
  if (m.gospelPct >= 8 && m.gospelPct <= 14) s += 1;
  if (!m.histStr?.startsWith('TOP40:')) s += 1;
  return s;
}

function main() {
  console.log('Wichita rock-trim diagnostics (Variant B base, in-vm only)\n');
  ...
  for (const variant of VARIANTS) {
    ...
    if (bad.length) console.error(`  ${bad.length} failed — ${bad[0]?.cellId}: ${bad[0]?.err}`);

    all[variant] = {};
    for (const p of PATHS) {
      all[variant][p.id] = { label: p.label, markets: {} };
      for (const mid of p.markets) {
        const s = summarize(
        ...
        all[variant][p.id].markets[mid] = s;
        if (!s || mid !== 'wichita') continue;
        const flags = [s.top40Mismatch, s.chrFlags].filter(Boolean).join('|');
        console.log(
          `${p.label}: ctry=${s.countryPct.toFixed(1)}% rock=${s.rockPct.toFixed(1)}% AC=${s.acPct.toFixed(1)}% talk=${s.talkPct.toFixed(1)}% gospel=${s.gospelPct.toFixed(1)}% urban=${s.urbanPct.toFixed(1)}% HHI≈${s.hhi.toFixed(0)}`,
        );
  ...
  console.log('\n========== Wichita 1985→2026 summary ==========\n');
  console.log('Var\tCountry\tRock\tAC\tTalk\tGospel\tUrban\tHHI\t#1\tScore');
  for (const variant of VARIANTS) {
  ...
  console.log('\n========== Wichita 1985→1995 ==========\n');
  console.log('Var\tCountry\tRock\tAC\t#1');
  for (const variant of VARIANTS) {
    const m = all[variant].w85_95?.markets?.wichita;
    console.log(
      [variant, m?.countryPct?.toFixed(1), m?.rockPct?.toFixed(1), m?.acPct?.toFixed(1), m?.histStr].join('\t'),
    );
  }

  console.log('\n========== 1970→1975 Wichita control ==========\n');
  console.log('Var\tCountry\tRock\t#1');
  for (const variant of VARIANTS) {
    const m = all[variant].w70_75?.markets?.wichita;
    console.log([variant, m?.countryPct?.toFixed(1), m?.rockPct?.toFixed(1), m?.histStr].join('\t'));
  }

  console.log('\n========== Nashville 1985→2026 (bleed check) ==========\n');
  for (const variant of VARIANTS) {
    const m = all[variant].n85_26?.markets?.nashville;
    console.log(`${variant}: ctry=${m?.countryPct?.toFixed(1)}% rock=${m?.rockPct?.toFixed(1)}% #1=${m?.histStr}`);
  }

  mkdirSync(path.join(root, 'tmp'), { recursive: true });
  const outPath = path.join(root, 'tmp', 'wichita_rock_trim_ab.json');
  writeFileSync(
    outPath,
    `${JSON.stringify(
      {
        ...
        runs: RUNS,
        seed: SEED,
        base: 'Variant B (MARKET_BP_PATCH.wichita + dialBpAmToFm)',
        variants: {
          B: 'baseline',
          E: 'BP7/9 ALBUM_ROCK str niche; BP15 CLASSIC_ROCK emerging',
          F: 'opening OQ ALBUM_ROCK+CLASSIC_ROCK FM ×0.86',
          G: 'post-1995 driftMod×0.90 + book appeal×0.90 on rock lanes',
          H: 'E+F+G',
        },
        results: all,
      },
