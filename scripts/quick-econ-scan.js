#!/usr/bin/env node
/**
 * Quick sanity-check of station EBITDA margins across markets and scenarios.
 * Read-only: VM-loads src/legacy.js, runs genMarket + seedRev.
 * Usage: node scripts/quick-econ-scan.js
 */
const fs = require('fs');
...
const MARKETS = [
  { id: 'atlanta', label: 'ATLANTA', revScaleNote: '~1' },
  { id: 'chicago', label: 'CHICAGO', revScaleNote: '~2.8' },
  { id: 'losangeles', label: 'LOS ANGELES', revScaleNote: '~5.2' },
];

const SCENARIOS = [
  { id: 'under', label: 'UNDERDOG' },
  { id: 'stack', label: 'STACK' },
];

const SEED = 424242;
const LARGE_MARKET_IDS = new Set(['chicago', 'losangeles']);
...
function runOne(marketId, scenId) {
...
function analyzeRun(marketLabel, scenLabel, rows) {
...
function pushWarnings(a, marketId, scenId) {
  const w = [];
...
function printSection(market, scen, rows) {
  const a = analyzeRun(market.label, scen.label, rows);
  pushWarnings(a, market.id, scen.id);

  console.log(`=== ${market.label} — ${scen.label} ===`);
...
function main() {
  console.log(`quick-econ-scan — seed ${SEED} (deterministic Math.random)\n`);
  for (const market of MARKETS) {
    for (const scen of SCENARIOS) {
      const rows = runOne(market.id, scen.id);
      printSection(market, scen, rows);
    }
  }
}
