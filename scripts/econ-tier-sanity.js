#!/usr/bin/env node
/**
 * Headless: one AM News/Talk + one FM AC per market, 1970 p1, calcRev+seedRev.
 * Optional: set ECON_DEBUG=1 to tag first AM/FM with G._econDebugIds for calcRev console rows.
 * Usage: node scripts/econ-tier-sanity.js [marketId]
 * Example: ECON_DEBUG=1 node scripts/econ-tier-sanity.js nashville
 */
const fs = require('fs');
...
const marketId = process.argv[2] || 'nashville';
const econDebug = process.env.ECON_DEBUG === '1' ? '1' : '0';
const shAm = marketId === 'nashville' ? '0.031' : '0.044';
const shFm = marketId === 'nashville' ? '0.02' : '0.024';

vm.runInContext(
  `
  var __ECON_DEBUG__='${econDebug}';
  (function(){
    var s = 999888;
  ...
  G = genMarket('wsb');
  while (!(G.year === 1970 && G.period === 1)) { advTurn(); }
  ...
  injectStation(amNt, ${shAm});
  injectStation(fmAc, ${shFm});
  if (__ECON_DEBUG__ === '1') {
    G._econDebugIds = [amNt.id, fmAc.id];
    G._econDebugLog = [];
  }
  ...
  globalThis.__out = {
    ...
    year: G.year,
    period: G.period,
    newsTalk: { call: amNt.callLetters, share: Math.round(amNt.rat.share * 1000) / 10, marginPct: marginPct(amNt), opsFloor: amNt.fin.opsFloor, tal: amNt.fin.tal, fix: amNt.fin.fix },
    fmAc: { call: fmAc.callLetters, share: Math.round(fmAc.rat.share * 1000) / 10, marginPct: marginPct(fmAc), opsFloor: fmAc.fin.opsFloor, tal: fmAc.fin.tal, fix: fmAc.fin.fix },
    econDebugLog: G._econDebugLog || null,
  };
  `,
  ctx
);

console.log(JSON.stringify(ctx.__out, null, 2));
