import{c as o,a as c,s as d,b as p}from"./clerkClientInit-Ql-mBo7x.js";import{e as x,g as m}from"./gameServerApiOrigin-CMVeZVq_.js";const i="pk_live_Y2xlcmsuYWlyd2F2ZWVtcGlyZS5jb20k".trim?.()??"";function u(){if(typeof window>"u")return"/#pricing";const{origin:t,pathname:n,search:e}=window.location;return`${t}${n&&n!==""?n:"/"}${e||""}#pricing`}function g(t){t.innerHTML=`
  ...
  `}function s(t){const n=document.getElementById("wl-index-pricing-clerk");if(!n)return;let e=document.getElementById("wl-index-pricing-notice");if(!t){e&&e.remove();return}e||(e=document.createElement("p"),e.id="wl-index-pricing-notice",e.className="mt-5 text-center text-[13px] text-stone-500 leading-relaxed max-w-md mx-auto",n.appendChild(e)),e.textContent=t}async function b(t,n){const e=x()[n];if(!e)return;if(!t?.session){s("Open Account to sign in, then return here to subscribe.");return}const a=await t.session.getToken().catch(()=>null);if(!a){s("Open Account to sign in, then return here to subscribe.");return}s("One moment…");const r=await fetch(m("/api/billing/create-checkout-session"),{method:"POST",headers:{Authorization:`Bearer ${a}`,"Content-Type":"application/json"},body:JSON.stringify({priceId:e})}).catch(()=>null),l=r?await r.json().catch(()=>({})):{};if(!r||!r.ok||!l.url){s(l.error||"Something went wrong. Try Account from the footer.");return}s(""),window.location.assign(l.url)}async function v(){const t=document.getElementById("wl-index-pricing-clerk");if(t){if(!i){t.innerHTML='<p class="text-stone-600 text-[13px] text-center leading-relaxed max-w-sm mx-auto">Memberships aren’t available on this preview. Continue in <a href="/account.html" class="text-amber-200/85 underline underline-offset-2 hover:text-amber-100">Account</a>.</p>';return}try{g(t),o(),await c(i,"Failed to load @clerk/ui");const n=new d(i,p()),e=u();await n.load({ui:{ClerkUI:window.__internal_ClerkUICtor},signInForceRedirectUrl:e,signUpForceRedirectUrl:e,signInFallbackRedirectUrl:e,signUpFallbackRedirectUrl:e}),t.querySelectorAll("button[data-price]").forEach(a=>{a.addEventListener("click",()=>b(n,a.getAttribute("data-price")))})}catch(n){console.error("[indexMarketing] pricing:",n),t.innerHTML='<p class="text-stone-600 text-[13px] text-center leading-relaxed">Could not load memberships. Try <a href="/account.html" class="text-amber-200/85 underline underline-offset-2">Account</a> or <a href="/play.html" class="text-amber-200/85 underline underline-offset-2">Play</a>.</p>'}}}v();
